...
// --- get() without version → latest variables ---

async () => {
  const result = await client.get('type-test-prompt');
  type Vars = Parameters<typeof result.userMessage>[0];
  type _Check = Expect<Equal<Vars, { city: string; country: string }>>;
};

// --- get() with known version → that version's variables ---

async () => {
  const result = await client.get('type-test-prompt', { version: '1.0.0' });
  type Vars = Parameters<typeof result.userMessage>[0];
  type _Check = Expect<Equal<Vars, { name: string }>>;
};

async () => {
  const result = await client.get('type-test-prompt', { version: '2.0.0' });
  type Vars = Parameters<typeof result.userMessage>[0];
  type _Check = Expect<Equal<Vars, { city: string; country: string }>>;
};

// --- get() with unknown version on known prompt → type error ---

async () => {
  // @ts-expect-error — '9.9.9' is not a codegen-known version
  await client.get('type-test-prompt', { version: '9.9.9' });
};

// --- get() with unknown promptId → Record<string, string> fallback (any version accepted) ---

async () => {
  const result = await client.get('unknown-prompt-id');
  type Vars = Parameters<typeof result.userMessage>[0];
  type _Check = Expect<Equal<Vars, Record<string, string>>>;
};

async () => {
  const result = await client.get('unknown-prompt-id', { version: '1.0.0' });
  type Vars = Parameters<typeof result.userMessage>[0];
  type _Check = Expect<Equal<Vars, Record<string, string>>>;
};
...
async () => {
  const v1 = await client.get('type-test-prompt', { version: '1.0.0' });
  // @ts-expect-error — v1.0.0 has { name }, not { city, country }
  v1.userMessage({ city: 'London', country: 'UK' });

  const latest = await client.get('type-test-prompt');
