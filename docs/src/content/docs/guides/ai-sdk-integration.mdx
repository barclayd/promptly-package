...
The `compose()` method runs all prompts in parallel and assembles the output in one line:

```typescript
const { compose } = await promptly.getComposer('article-builder', {
  input: { topic: 'TypeScript', audience: 'developers' },
});

const article = await compose(generateText);
```

Override parameters for every prompt via a wrapper:

```typescript
const article = await compose((prompt) =>
  generateText({ ...prompt, maxTokens: 500 })
);
```

<Aside>
  See [Fetching Composers](/guides/fetching-composers/) for the full guide. For per-prompt control, streaming, and structured output, see [Advanced Composer Patterns](/guides/advanced-composer-patterns/).
</Aside>
