...
// Trusted raw HTML
formatComposer({
  introPrompt: { html: '<p>Manually written<br>intro.</p>' },
  reviewPrompt: { html: '<strong>Static fallback HTML.</strong>' },
});
...
String and `{ text }` values preserve newlines as `<br>` tags. Use `{ html }`
when the prompt result already contains trusted HTML and should pass through
unchanged.
