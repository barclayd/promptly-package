...
const composer = await promptly.getComposer('article-composer', {
...
`compose()` calls your generate function for each prompt, runs them in parallel via `Promise.all`, and assembles the static HTML + AI text into a single string.
...
## `compose()` — the simple path

The `compose()` method handles the entire flow: it runs your generate function for each prompt in parallel and assembles the result:

```typescript
const output = await composer.compose(generateText);
```

### With per-prompt overrides

Pass a wrapper function to customise parameters:
...
For advanced use cases (different settings per prompt, streaming, structured output), use the manual flow with named prompts and `formatComposer()` described in the sections below.
