...
<Aside>
  The `userMessage` in prompt segments contains raw template strings with `${variable}` placeholders, just like prompts. The [SDK](/reference/client-api/#clientgetcomposercomposerid-options) handles interpolation automatically via the `input` option.
</Aside>
