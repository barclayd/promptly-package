...
`compose()` accepts any function matching `(prompt: ComposerPrompt) => Promise<FormatInput>`. String and `{ text }` results preserve newlines as `<br>` tags. Return `{ html }` only when you want trusted HTML inserted unchanged.
