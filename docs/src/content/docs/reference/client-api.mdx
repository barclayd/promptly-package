...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `composerId` | `ComposerId` | Yes | The composer ID to fetch. Narrows to generated composer IDs if codegen types are present. |
| `options.version` | `string` | No | Specific version to fetch (default: latest) |
| `options.input` | `Record<string, unknown>` | No | Input variables to interpolate into static segments and prompt user messages |
...
`compose()` accepts any function matching `(prompt: ComposerPrompt) => Promise<FormatInput>`. String and `{ text }` results preserve newlines as `<br>` tags and keep visible paragraph gaps when inserted into standalone rich-text paragraphs. Return `{ html }` only when you want trusted HTML inserted unchanged.

### `formatComposer()` usage

The `formatComposer()` function accepts a record mapping prompt names to results. Each value can be a string, an object with a `text` property (matching the `generateText()` return shape), or an object with an `html` property:
...
// Or pass trusted HTML without newline conversion
const output = formatComposer({
  introPrompt: { html: '<p>Hello<br>world</p>' },
  reviewPrompt: { html: '<strong>Looks good.</strong>' },
});
```
...
```typescript
client.getComposer<
  T extends ComposerId,         // Composer ID
...
| Property | Type | Required | Description |
|----------|------|----------|-------------|
| `composerId` | `ComposerId` | Yes | The composer ID |
| `input` | `Record<string, unknown>` | No | Input variables for interpolation |
| `version` | `string` | No | Specific version (default: latest) |
