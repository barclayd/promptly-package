# Build `@promptlycms/prompts` — TypeScript SDK for Promptly CMS

Build an npm package called `@promptlycms/prompts` — a TypeScript SDK for the Promptly CMS API.
...
### 1. Runtime Client (`@promptlycms/prompts`)
...
```
@promptlycms/prompts           → runtime client (createPromptClient, types)
@promptlycms/prompts/schema    → buildZodSchema utility (for advanced use)
@promptlycms/prompts/generate  → CLI entry point
```
