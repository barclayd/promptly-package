...
- [#28](https://github.com/barclayd/promptly-package/pull/28) [`8246366`](https://github.com/barclayd/promptly-package/commit/824636648b6f600e1b4d5e0b377dc72dd6819e62) Thanks [@barclayd](https://github.com/barclayd)! - Codegen now warns when composer fetch fails instead of silently dropping types from `promptly-env.d.ts`. The generated file always emits `ComposerVariableMap` and `ComposerPromptMap` interfaces (empty when no composers are returned) so its shape stays stable run-to-run.
